...
import { createRelayClient, RelayClient } from '../sdk/relay-client';
...
program
  ...
  .action(async (options) => {
    ...
    const client = await createRelayClient(`agent_${Date.now()}`);
    ...
    const keyPair = {
      publicKey,
      privateKey: '*** STORED SECURELY ***', // In production, use proper key storage
    };
